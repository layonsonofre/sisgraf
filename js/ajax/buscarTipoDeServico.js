$(function() {
    ...
    var request;
    $("#buscarTipoDeServico").submit(function(event){
        ...
        request.done(function (response, textStatus, jqXHR){
            ...
            $('#resultadoBuscaTipoDeServico').empty().append(response);
        });
    ...
    $(document).on("click", "#remover",function(event) {
        ...
        if(confirm('Tem certeza que deseja excluir este tipo de serviço?')) {
            request = $.ajax({
                url: "control/tipoDeServico.php",
                type: "post",
                data: "acao=excluir&idTipoServico="+idPessoa
            });
            request.done(function (response, textStatus, jqXHR) {
                console.log(response);
                $("#buscarTipoDeServico")[0].click();
            });
            request.fail(function (jqXHR, textStatus, errorThrown){
                console.error(
                    "The following error occurred: "+
